<?php
/**
 * Draw stock : amount-start-high-low-end price graph
 */
...
if(!$this->scale['values'])
{
	$r_start_value = $this->data[0][3];
	$r_end_value = $this->data[0][2];
	for($i = 1; $i < count($this->data); $i++)
	{
		$r_start_value = $r_start_value > $this->data[$i][3] ? $this->data[$i][3] : $r_start_value;
		$r_end_value = $r_end_value < $this->data[$i][2] ? $this->data[$i][2] : $r_end_value;
	}
	$r_field_number = 6;
...
$a_column_width = $unitx * (1 - 2 * $a_gap);
$a_column_gap = $unitx * $a_gap;
$p_column_width = $unitx * (1 - 2 * $p_gap);
$p_column_gap = $unitx * $p_gap;
...
{
	$a_columnx = $startx + $unitx * $i + $a_column_gap;
	$l_y_field = 0;
	...
	# fill column
	imagefilledrectangle($this->chart, $a_columnx, $columny, $a_columnx + $a_column_width, $starty - 1, $scale_color);
	# column border
	imageline($this->chart, $a_columnx, $starty - 1, $a_columnx, $columny, $border_color);
	imageline($this->chart, $a_columnx, $columny, $a_columnx + $a_column_width, $columny, $border_color);
	imageline($this->chart, $a_columnx + $a_column_width, $columny, $a_columnx + $a_column_width, $starty - 1, $border_color);
}

# draw price lines & rectangles
imagesetthickness($this->chart, $this->graph['shadow'] * 10 * 2);
for($i = 0; $i < count($this->data); $i++)
{
	$linex = $startx + $unitx * ($i + 0.5);
	$p_columnx = $startx + $unitx * $i + $p_column_gap;
	
	$y_start_field = $y_high_field = $y_low_field = $y_end_field = 0;
	for($j = 0; $j < $r_field_number - 1; $j++)
	{
		if(!$y_start_field && $this->data[$i][1] > $this->scale['values'][1][$j] && $this->data[$i][1] <= $this->scale['values'][1][$j + 1])
		{
			$y_start_field = $j;
		}
		if(!$y_high_field && $this->data[$i][2] > $this->scale['values'][1][$j] && $this->data[$i][2] <= $this->scale['values'][1][$j + 1])
		{
			$y_high_field = $j;
		}
		if(!$y_low_field && $this->data[$i][3] > $this->scale['values'][1][$j] && $this->data[$i][3] <= $this->scale['values'][1][$j + 1])
		{
			$y_low_field = $j;
		}
		if(!$y_end_field && $this->data[$i][4] > $this->scale['values'][1][$j] && $this->data[$i][4] <= $this->scale['values'][1][$j + 1])
		{
			$y_end_field = $j;
		}
	}
	$y_start = $starty - $r_unity * ($y_start_field + ($this->data[$i][1] - $this->scale['values'][1][$y_start_field]) / ($this->scale['values'][1][$y_start_field + 1] - $this->scale['values'][1][$y_start_field]));
	$y_high = $starty - $r_unity * ($y_high_field + ($this->data[$i][2] - $this->scale['values'][1][$y_high_field]) / ($this->scale['values'][1][$y_high_field + 1] - $this->scale['values'][1][$y_high_field]));
	$y_low = $starty - $r_unity * ($y_low_field + ($this->data[$i][3] - $this->scale['values'][1][$y_low_field]) / ($this->scale['values'][1][$y_low_field + 1] - $this->scale['values'][1][$y_low_field]));
	$y_end = $starty - $r_unity * ($y_end_field + ($this->data[$i][4] - $this->scale['values'][1][$y_end_field]) / ($this->scale['values'][1][$y_end_field + 1] - $this->scale['values'][1][$y_end_field]));
	# draw line
	imageline($this->chart, $linex, $y_low, $linex, $y_high, $border_color);
	# draw rectangle
	if($this->data[$i][1] <= $this->data[$i][4])
	{
		imagefilledrectangle($this->chart, $p_columnx, $y_end, $p_columnx + $p_column_width, $y_start, $up_color);
		imagefilledrectangle($this->chart, $p_columnx + 1, $y_end + 1, $p_columnx + $p_column_width - 1, $y_start - 1, $up_bg);
	}
	else
	{
		imagefilledrectangle($this->chart, $p_columnx, $y_start, $p_columnx + $p_column_width, $y_end, $down_color);
		imagefilledrectangle($this->chart, $p_columnx + 1, $y_start + 1, $p_columnx + $p_column_width - 1, $y_end - 1, $down_bg);
	}
}
?>